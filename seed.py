...
try:
    from content.frog_jokes_200 import JOKES_200
except ImportError:
    JOKES_200 = []

try:
    from content.frog_facts_200 import FACTS_200
except ImportError:
    FACTS_200 = []
...
async def seed(db_path: str, frogs_json_path: str = DEFAULT_JSON):
    ...
    # 2) Jokes: 200 харківських жартів (content/frog_jokes_200.py), інакше базові 5
    row = await db.fetchone("SELECT COUNT(1) AS c FROM jokes")
    jokes_count = int(row["c"]) if row else 0
    if JOKES_200 and jokes_count < 200:
        for j in JOKES_200:
            await db.execute("INSERT INTO jokes(text, tags) VALUES(?,?)", (j, "kharkiv"))
        print(f"[seed] jokes imported: {len(JOKES_200)}")
    elif jokes_count < 20:
        for j in JOKES:
    ...
    # 4) Facts: 200 фактів про жаб, болото, воду, головастиків (content/frog_facts_200.py)
    row = await db.fetchone("SELECT COUNT(1) AS c FROM facts")
    facts_count = int(row["c"]) if row else 0
    if FACTS_200 and facts_count < 200:
        for f in FACTS_200:
            await db.execute("INSERT INTO facts(text, tags) VALUES(?,?)", (f, "frog"))
        print(f"[seed] facts imported: {len(FACTS_200)}")
